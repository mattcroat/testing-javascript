...
let MultiPageForm = React.createContext()

function MultiPageFormProvider({ initialValues = {}, ...props }) {
  let [initState] = React.useState(initialValues)
  let [form, setFormValues] = React.useReducer(
    (s, a) => ({ ...s, ...a }),
    initState
  )
  let resetForm = () => setFormValues(initialValues)
...
function useMultiPageForm() {
  let context = React.useContext(MultiPageForm)
  if (!context) {
    throw new Error(
...
function Page1() {
  let { form, setFormValues } = useMultiPageForm()
...
function Confirm() {
  let { form, resetForm } = useMultiPageForm()
